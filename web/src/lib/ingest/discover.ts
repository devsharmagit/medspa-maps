...
/**
 * Return up to 3 extra content-page URLs (locations, contact, about) to fetch.
 * Excludes the homepage; all same-registrable-host.
 */
export async function discoverContentPages(
  ...
  for (const u of [nav.contact, nav.about]) if (u) candidates.add(u);
  ...
  for (const u of [pick(LOC_RE), pick(CONTACT_RE), pick(ABOUT_RE)]) {
    if (!u) continue;
    const p = path(u);
