      ...
      <div className="grid gap-8 lg:grid-cols-3">
         {currentProviders.map((p) => {
           ...
           return (
             <div
               key={p.id}
               className="flex bg-white shadow-[0px_6px_10.5px_1px_rgba(0,0,0,0.05)] rounded-[22px] overflow-hidden"
             >
               ...
               <div className="w-1/2 p-6 flex flex-col justify-between">
                 <div className="flex flex-col gap-2">
                   <div className="flex items-center gap-2">
                     <h3 className="text-[18px] font-medium leading-[116%] tracking-[0.02em] text-[#383838]">
                       {p.name}
                     </h3>
                     {p.is_verified && (
                   ...
                   {p.title && (
                     <p className="text-[14px] leading-[138%] tracking-[0.02em] text-[#727272]">
                       {p.title}
                     </p>
                   )}
                 </div>
                 ...
                 <Link
                   href={`/providers/${p.id}/${p.name.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`}
                   className="mt-4 flex items-center justify-center w-full py-2.5 bg-gradient-to-r from-[#DE7F4C] to-[#C341D7] rounded-[8px] text-[14px] font-semibold text-white transition hover:opacity-95"
                 >
                   View Profile
                 </Link>
               </div>
