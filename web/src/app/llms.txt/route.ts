...
export async function GET() {
  ...
  const lines = [
    `# ${SITE_NAME}`,
    "",
    `> ${SITE_NAME} is a free, editorially-vetted directory of medspas across the United States. Every listing pairs the treatments and concerns a practice actually offers with real patient reviews, hours, and location — alongside plain-English treatment guides — so patients can compare practices on facts and book directly.`,
    "",
    "## Key pages",
    `- [Search medspas](${absoluteUrl("/search")}): Find and compare medspas by treatment, concern, or location (e.g. ${absoluteUrl("/search?location=TX")}).`,
    `- [Patients' Favourites](${absoluteUrl("/patients-favourites")}): Popular treatment and condition guides patients research most.`,
    `- [Conditions](${absoluteUrl("/conditions")}): Skin and aesthetic concerns, and the medspas that treat them.`,
